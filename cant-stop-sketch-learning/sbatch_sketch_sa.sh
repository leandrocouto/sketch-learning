...
#declare -a evals=("EvalStateBasedImitationAgent" "EvalStateBasedImitationGlennAgent" "EvalStateBasedImitationRandomAgent" "EvalActionBasedImitationAgent")
				...
				for n in 1000; do
					for eval in ${evals[@]}; do
						for param in ${params[@]}; do
							...
							sbatch --output=${output_exp} --export=log_file="${log_file}",param="${param}",temp=${temp},alpha=${alpha},beta=${beta},program_file=${program_file},eval=${eval},n=${n} run_sketch_sa.sh
						done
					done
				done
			done

	...
	for n in 1000; do
		for eval in ${evals[@]}; do
			for param in ${params[@]}; do
				...
				output_exp="${output}rwsp-${lower_eval}-n${n}-${name_param}-v${iter}"
				log_file="log-${lower_eval}-n${n}-${name_param}-v${iter}"
				program_file="programs-${lower_eval}-n${n}-${name_param}-v${iter}"
	
				sbatch --output=${output_exp} --export=log_file="${log_file}",param="${param}",program_file=${program_file},eval=${eval},n=${n} run_rwsp.sh
			done
		done
	done
done

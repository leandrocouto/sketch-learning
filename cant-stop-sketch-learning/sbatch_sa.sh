				...
				for n in 1000; do
					for eval in ${evals[@]}; do
						for param in ${params[@]}; do
							...
							output_exp="${output}sa-${lower_eval}-beta${beta}-alpha${alpha_name}-temp${temp}-n${n}-${name_param}-v${iter}"
							...
							sbatch --output=${output_exp} --export=log_file="${log_file}",param="${param}",temp=${temp},alpha=${alpha},beta=${beta},program_file=${program_file},eval=${eval},n=${n} run_sa.sh
						done
					done
				done
			done

...
declare -a params=("--triage--double-programs--reuse-tree" "--triage--double-programs" "--triage--bidirectional--double-programs" "--triage--bidirectional--double-programs--reuse-tree")
declare -a sim_functions=("SA" "Random")
						...
						for eval in ${evals[@]}; do
							for sim_function in ${sim_functions[@]}; do
								for param in ${params[@]}; do
									...
									output_exp="${output}ibr-uct-${lower_eval}-beta${beta}-alpha${alpha_name}-n${n}-c${c_name}-${lower_sim}${name_param}-v${iter}"
									...
									sbatch --output=${output_exp} --export=log_file="${log_file}",param="${param}",sim_function=${sim_function},alpha=${alpha},beta=${beta},program_file=${program_file},eval=${eval},n=${n},sims=${sims},c=${c} run_ibr_uct.sh
								done
							done
						done
					done

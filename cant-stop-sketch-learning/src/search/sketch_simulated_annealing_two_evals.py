...
import os
from evaluation import EvalStateBasedImitationAgent, EvalStateBasedImitationGlennAgent, EvalStateBasedImitationRandomAgent, EvalActionBasedImitationAgent
import pickle

class SketchSimulatedAnnealingTwoEvals():
    ...
    def __init__(self, log_file, program_file):
        ...
        self.log_file_imit = 'twoevals-sketch-sa-' + log_file
    ...
    def return_terminal_child(self, p, types):
        ...
        for t in types:
            ...
            if child.get_number_children() == 0 or isinstance(child, NumericConstant) or isinstance(child, StringConstant) or isinstance(child, VarList) or isinstance(child, VarScalar) or isinstance(child, VarScalarFromArray):
    ...
    def fill_random_program(self, p, depth, max_depth):
        ...
        for i in range(p.get_number_children()):
            ...
            if isinstance(p, NumericConstant) or isinstance(p, StringConstant) or isinstance(p, VarList) or isinstance(p, VarScalar) or isinstance(p, VarScalarFromArray):
    ...
    def log_results_imit(self, score):
        with open(join(self.log_folder + self.log_file_imit), 'a') as results_file:
            results_file.write(("{:d}, {:f}, {:f} \n".format(self.id_log, score, time.time() - self.time_start)))
    ...
    def search(self, 
        ...
        self.best_score_imit = None
        
        for _ in range(5):
            eval_sketch_imitation = self.eval_function #EvalStateBasedImitationAgent()
            ...
            if best_program_imitation is not None:
                ...
                if self.best_program is None or score_against_glenn > self.best_score:
                    ...
                    self.best_score_imit = score
                    
                    self.log_results(self.best_score, self.best_program)
                    self.log_results_imit(score)
        ...
        while True:
            ...
            for i in range(len(self.initial_programs)):
                ...
                if self.best_score is None or score > self.best_score:                   
                    ...
                    imit_score, _, _ = self.eval_function.eval(program)
                    self.best_score_imit = imit_score
                    self.log_results_imit(imit_score)
                
                if time.time() - self.time_start > time_limit - self.slack_time:
                    self.log_results(self.best_score, self.best_program)
                    self.log_results_imit(self.best_score_imit)
...
#                     pickle.dump(best_program, file_program)
            ...
            if isinstance(eval_function, EvalStateBasedImitationAgent) or \
                isinstance(eval_function, EvalStateBasedImitationGlennAgent) or \
                isinstance(eval_function, EvalStateBasedImitationRandomAgent) or \
                isinstance(eval_function, EvalActionBasedImitationAgent):

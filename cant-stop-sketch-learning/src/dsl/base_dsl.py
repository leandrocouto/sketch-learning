...
class Node:
    ...
    def new(self, *args):
        raise Exception('Unimplemented method: new')
    ...
    def filter_production_rules(operations,
        ...
        for op in list_all_productions:
            ...
            for types in op.accepted_types:
                for t in types:
                    if t not in rules:
                        op_to_remove.append(t)

                for t in op_to_remove:
                    if t in types:
                        types.remove(t)

    @staticmethod
    def restore_original_production_rules():
        DoubleProgram.accepted_nodes = {ITE.class_name(), Argmax.class_name(), Sum.class_name(),
                                        Plus.class_name(), Times.class_name(), Minus.class_name()}
        DoubleProgram.accepted_types = [DoubleProgram.accepted_nodes, DoubleProgram.accepted_nodes]

        Times.accepted_nodes = {VarScalar.class_name(), VarScalarFromArray.class_name(),
                                NumberAdvancedThisRound.class_name(),
                                NumberAdvancedByAction.class_name(), NumericConstant.class_name(),
                                Plus.class_name(), Times.class_name(), Minus.class_name(),
                                Sum.class_name(), PositionsOpponentHasSecuredInColumn.class_name(),
        ...
        Minus.accepted_nodes = {VarScalar.class_name(), VarScalarFromArray.class_name(),
                                NumberAdvancedThisRound.class_name(),
                                NumberAdvancedByAction.class_name(), NumericConstant.class_name(),
                                Plus.class_name(), Times.class_name(), Minus.class_name(),
                                Sum.class_name(), PositionsOpponentHasSecuredInColumn.class_name(),
        ...
        Plus.accepted_nodes = {VarScalar.class_name(), VarScalarFromArray.class_name(),
                               NumberAdvancedThisRound.class_name(),
                               NumberAdvancedByAction.class_name(), NumericConstant.class_name(),
                               Plus.class_name(), Times.class_name(), Minus.class_name(),
                               Sum.class_name(), PositionsOpponentHasSecuredInColumn.class_name(),
        ...
        Function.accepted_nodes = {Minus.class_name(), Plus.class_name(), Times.class_name(),
                                   Sum.class_name(), Map.class_name()}

        Function.accepted_types = [Function.accepted_nodes]

        AssignActionToReturn.accepted_nodes = {Argmax.class_name(), NumericConstant.class_name()}
        AssignActionToReturn.accepted_types = [AssignActionToReturn.accepted_nodes]

        ITE.accepted_nodes_bool = {LT.class_name()}
        ITE.accepted_nodes_block = {Sum.class_name(), Argmax.class_name(), Plus.class_name(),
                                    Times.class_name(), Minus.class_name(), ITE.class_name()}
        ITE.accepted_types = [ITE.accepted_nodes_bool, ITE.accepted_nodes_block,
                              ITE.accepted_nodes_block]

        In.accepted_nodes_left = {VarScalar.class_name(), StringConstant.class_name()}
        In.accepted_nodes_right = {VarList.class_name()}
        In.accepted_types = [In.accepted_nodes_left, In.accepted_nodes_right]

        LT.accepted_nodes = {NumericConstant.class_name(), Plus.class_name(), Times.class_name(),
                             Minus.class_name(), Sum.class_name()}

        LT.accepted_types = [LT.accepted_nodes, LT.accepted_nodes]

        Argmax.accepted_nodes = {Map.class_name(), VarList.class_name(), LocalList.class_name()}
        Argmax.accepted_types = [Argmax.accepted_nodes]

        Sum.accepted_nodes = {Map.class_name(), VarList.class_name(), LocalList.class_name()}
        Sum.accepted_types = [Sum.accepted_nodes]

        Map.accepted_nodes_function = {Function.class_name()}
        Map.accepted_nodes_list = {VarList.class_name(), Map.class_name(), LocalList.class_name()}
        Map.accepted_types = [Map.accepted_nodes_function, Map.accepted_nodes_list]

        Node.accepted_types = [{ITE.class_name(), AssignActionToReturn.class_name()}]
...
class ITE(Node):
    ...
    def to_string(self):
        return 'if ' + self.children[0].to_string() + ' then: ' + self.children[
            1].to_string() + ' else: ' + self.children[2].to_string()

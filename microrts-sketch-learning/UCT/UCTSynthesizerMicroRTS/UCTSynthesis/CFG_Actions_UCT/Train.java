...
public class Train extends Node {
	...
	public Train(Node type, Node direc,Node n) {
		this.used=false;
	...
	public String translate() {
		...
		return "u.train("+this.children.get(0).getSValue()+","+this.children.get(1).getSValue()+","+this.children.get(2).getSValue()+")";
	}
	...
	public void interpret(GameState gs, int player, Unit u, Interpreter automato) throws Exception {
    	...
    	if( u.getPlayer() == player && u.getType().name.equals("Base")  && UType.name=="Worker" && automato.getAbstractAction(u)==null){
        	...
        	if (automato.resource >= UType.cost && this.contador(gs, player, u, automato)) {
        		Direction curr_d = (Direction)this.children.get(1);
			 	automato.train(u, UType,curr_d.converte(u,gs,player));
			 	this.used=true;
	...
	public Node Clone(Factory f) {
		// TODO Auto-generated method stub
		return f.build_Train(this.children.get(0).Clone(f),this.children.get(1).Clone(f),this.children.get(2).Clone(f));
	}

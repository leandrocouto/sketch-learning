...
public class EvaluationimtationAction implements EvaluationFunction {

	
	Oracle oracle;
	
	public EvaluationimtationAction(Oracle O) {
	...
	public Pair<Double, Double> run(GameState gs, int player, int max_cycle, AI ai1, AI ai2, boolean exibe)
		...
		double cont=0;
        for(int i=0; i<oracle.gss.size()-1;i++) {
        	PlayerAction pa1=null;
        	GameState gs2 = oracle.gss.get(i).cloneChangingUTT(utt);
        	try {
        	 ...
        	 cont+=compara(pa1,oracle.pa0s.get(i));
        	
        	
             
        }
        if(itbroke)return new Pair<>(0.0,0.0);
        double r2 =cont/(oracle.gss.size()-1);
